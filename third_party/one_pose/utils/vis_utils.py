...
def make_matching_plot(
    ...
    if fast_viz:
        make_matching_plot_fast(image0, image1, kpts0, kpts1, mkpts0, mkpts1, color, text, path, show_keypoints)
        return
    ...
    fig.text(0.01, 0.01, "\n".join(small_text), transform=fig.axes[0].transAxes, fontsize=5, va="bottom", ha="left", color=txt_color)
    if path:
...
def make_matching_plot_fast(image0, image1, kpts0, kpts1, mkpts0, mkpts1, color, text, margin=10, show_keypoints=True, num_matches_to_show=None):
    ...
    for i, t in enumerate(text):
        cv2.putText(out, t, (int(8 * scale), Ht * (i + 1)), cv2.FONT_HERSHEY_DUPLEX, 1.0 * scale, text_color_bg, 3, cv2.LINE_AA)
        cv2.putText(out, t, (int(8 * scale), Ht * (i + 1)), cv2.FONT_HERSHEY_DUPLEX, 1.0 * scale, text_color_fg, 1, cv2.LINE_AA)
...
def vis_match_pairs(pred, feats0, feats1, name0, name1):
    ...
    make_matching_plot_fast(image0, image1, kpts0, kpts1, mkpts0, mkpts1, color, text=[])
...
def draw_3d_box(image, corners_2d, linewidth=3, color="g"):
    ...
    """
    lines = np.array([[0, 1, 5, 4, 2, 3, 7, 6, 2, 2, 3, 7], [1, 5, 4, 0, 3, 0, 6, 5, 1, 6, 7, 4]]).T
...
def draw_2d_box(image, corners_2d, linewidth=3):
    ...
    box_pts = [[(x1, y1), (x1, y2)], [(x1, y2), (x2, y2)], [(x2, y2), (x2, y1)], [(x2, y1), (x1, y1)]]
...
def draw_reprojection_pair(data, val_results, visual_color_type="conf"):
    ...
    figure = make_matching_plot(query_image, query_image, mkpts2d, mkpts3d_reprojed, mkpts2d, mkpts3d_reprojed, color=color, text=text)
    figures["evaluation"].append(figure)

    return figures


def vis_reproj(image_full_path, poses, box3d_path, intrin_full_path, save_demo=False, demo_root=None, colors=["y", "g"]):
    """
...
def dump_wis3d(idx, cfg, data_dir, image0, image1, image_full, kpts2d, kpts2d_reproj, confidence, inliers):
    """Visualize by wis3d"""
    ...
    wis3d.add_keypoint_correspondences(
        ...
        kpts2d,
        kpts2d_reproj,
        metrics={"mconf": confidence.tolist(), "reproj_distance": reproj_distance.tolist()},
...
def vis_keypoints(image, kpts, color="r", radius=3):
    """Visualize keypoints"""
    color_dict = {"g": (0, 255, 0), "r": (0, 0, 255), "b": (255, 0, 0)}
    for kpt in kpts:
